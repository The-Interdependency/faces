...
const EMPTY_TEST_ROOT = mkdtempSync(join(tmpdir(), "grok-pwa-test-root-"));

/** Keep repository identity files from leaking into otherwise pure unit cases. */
function injectGrokPwaHead(html, context = {}) {
  return rawInjectGrokPwaHead(html, { cwd: EMPTY_TEST_ROOT, ...context });
}

function createHeadInjector(context = {}) {
  return rawCreateHeadInjector({ cwd: EMPTY_TEST_ROOT, ...context });
}
